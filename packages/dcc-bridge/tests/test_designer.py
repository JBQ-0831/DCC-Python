...
import dcc_bridge.setup.designer as designer
from dcc_bridge.setup.base import DCCInstallation, get_setup
from dcc_bridge.setup.designer import DesignerSetup, SD_REG_BASE
...
def mock_registry(monkeypatch):
    ...
    monkeypatch.setattr(DesignerSetup, "_read_registry_value", mock_read)
    return values


@pytest.fixture
def populated_registry(mock_registry):
    """预填充 Designer 的 App Paths 注册表数据"""
    mock_registry[SD_REG_BASE] = {
        "": r"C:\Program Files\Adobe\Adobe Substance 3D Designer\Adobe Substance 3D Designer.exe",
        "Path": r"C:\Program Files\Adobe\Adobe Substance 3D Designer\\",
    }
...
def mock_version_info(monkeypatch):
    ...
    def install(major: int, minor: int):
        ...
        monkeypatch.setattr(designer, "ctypes", fake)
        # 仅对 exe 路径返回 True，避免破坏 os.makedirs（其内部会调用
        # os.path.exists(head) 判断父目录是否已存在）
        _real_exists = designer.os.path.exists
        monkeypatch.setattr(
            designer.os.path, "exists", lambda p: str(p).endswith(".exe") or _real_exists(p)
        )
...
class TestDiscoverVersions:
    ...
    def test_returns_empty_when_exe_not_exists(self, setup_instance, populated_registry, monkeypatch):
        ...
        monkeypatch.setattr(designer.os.path, "exists", lambda p: False)
        assert setup_instance.discover_versions() == []
...
class TestGetInstallPath:
    def test_returns_install_dir(self, setup_instance, populated_registry):
        ...
        assert "Adobe Substance 3D Designer" in path
...
class TestGetScriptDir:
    def test_returns_sduserplugins_dir(self, setup_instance, populated_registry, mock_home):
        """脚本目录应为 .../python/sduserplugins"""
        result = setup_instance.get_script_dir("10.1")
        assert result is not None
        assert result.endswith(
            os.path.join("Adobe", "Adobe Substance 3D Designer", "python", "sduserplugins")
        )

    def test_dir_not_version_dependent(self, setup_instance, populated_registry, mock_home):
        """SD 单版本安装，脚本目录不随版本号变化"""
    ...
    def test_returns_dir_when_nothing_installed(self, setup_instance, mock_registry, mock_home):
        ...
        assert result is not None
        assert "sduserplugins" in result
...
class TestGetPythonPath:
    def test_returns_pythonsdk_path(self, setup_instance, populated_registry):
        """应拼接 plugins/pythonsdk/python.exe"""
        result = setup_instance.get_python_path("10.1")
        assert result is not None
        assert result.endswith(os.path.join("plugins", "pythonsdk", "python.exe"))
...
class TestGetStartupScriptContent:
    def test_contains_dcc_name(self, setup_instance):
        """内容应包含 substance_designer 标识"""
        assert "substance_designer" in setup_instance.get_startup_script_content()
...
class TestDetectInstallations:
    def test_returns_installation(self, setup_instance, populated_registry, mock_version_info):
        ...
        assert inst.dcc_name == "substance_designer"
...
class TestGetStartupScriptPath:
    def test_returns_full_path(self, setup_instance, populated_registry, mock_home):
        """应返回 sduserplugins 目录下的 dcc_bridge_startup.py"""
...
class TestGetSetupFactory:
    def test_returns_designer_setup_for_substance_designer(self):
        """get_setup('substance_designer') 应返回 DesignerSetup 实例"""
        assert isinstance(get_setup("substance_designer"), DesignerSetup)

    def test_returns_designer_setup_for_alias(self):
        """get_setup('designer') 也应返回 DesignerSetup 实例"""
        assert isinstance(get_setup("designer"), DesignerSetup)
