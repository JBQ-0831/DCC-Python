...
class DCCClient:
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = 7002, timeout: float = 30.0):
        self.host = host
        self.port = port
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            raise DCCClientError(f"Failed to connect to DCC at {self.host}:{self.port}: {e}") from e
    ...
    def execute_file(self, file_path: str, exec_origin: Optional[str] = None, name_var: str = "__main__") -> Response:
        """请求 DCC 执行本地文件"""
        exec_origin = exec_origin or file_path
        return self._send_request("execute", {
            "exec_file": file_path,
    ...
    def reload_modules(self, workspace_folders: List[str]) -> Response:
        """请求 DCC 重载工作区模块"""
        return self._send_request("reload", {
            "workspace_folders": workspace_folders,
        })

    def start_debugpy(self, port: int, python_path: Optional[str] = None) -> Response:
    ...
    def install_debugpy(self, python_path: Optional[str] = None, pip_index_url: str = "") -> Response:
        """请求 DCC 安装 debugpy"""
...
def resolve_client(
    ...
    """
    根据 dcc_type / port / version 自动解析目标并创建 DCCClient。
    ...
    if not instances:
        msg = f"No running DCC instance found"
        if dcc_type:
            msg += f" for type '{dcc_type}'"
        if version:
    ...
    if len(instances) > 1:
        raise DCCClientError(
            f"Multiple DCC instances found: "
            + ", ".join(
                f"{i.get('dcc_type')}:{i.get('dcc_version', '?')}:{i.get('port')}"
                for i in instances
            )
            + ". Please specify --port, --dcc-type, or --version."
        )
