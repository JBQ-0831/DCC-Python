...
class TestGetStartupScriptPath:
    def test_returns_full_path(self, setup_instance, populated_registry, mock_home):
        ...
        assert path.endswith(
            os.path.join("python", "startup", "dcc_bridge", "dcc_bridge_startup.py")
        )
...
class TestSetup:
    def test_writes_startup_script(self, setup_instance, populated_registry, mock_home):
        ...
        script_path = mock_home / "Documents" / "Adobe" / "Adobe Substance 3D Painter" / "python" / "startup" / "dcc_bridge" / "dcc_bridge_startup.py"
...
class TestLauncher:
    def test_writes_launcher(self, setup_instance, populated_registry, mock_home):
        ...
        assert setup_instance.setup("10.1") is True
        launcher = (
            mock_home / "Documents" / "Adobe" / "Adobe Substance 3D Painter"
            / "python" / "startup" / "dcc_bridge_launcher.py"
        )
